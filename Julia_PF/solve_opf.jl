...
The decision variable vector is partitioned into blocks as follows:
    ...
    18. Cct_dc  -  VSC SOC relaxed term no.5 
...
function setup_ac(model::JuMP.Model,
    ...
    for ng in 1:ngrids
        ...
        # Consider only the upper triangular part for the relaxation by using a mask matrix
...
This function defines the AC and VSC coupling constraints for power injections
and voltage.
...
function setup_cp(model::JuMP.Model,
    ...
    for grid in 1:ngrids
        # These are initialized as affine expressions with zero constant.
        pm_ac = [AffExpr(0.0) for _ in 1:nbuses_ac[grid]]
        qm_ac = [AffExpr(0.0) for _ in 1:nbuses_ac[grid]]
        
        #  Every AC node have load 
        pm_ac .= pm_ac .- pd_ac[grid]
        qm_ac .= qm_ac .- qd_ac[grid]
        
        #  If the AC node connected with generator
        for i in 1:ngens_ac[grid]
            bus_index = Int(generator_ac[grid][i, 1])
            pm_ac[bus_index] += pgen_ac[grid][i]
            qm_ac[bus_index] += qgen_ac[grid][i]
        end
        
        # If the AC node connected with VSC
        for i in 1:nconvs_dc
            if Int(conv_dc[i, 3]) == grid
        ...
        @constraint(model, pn_ac[grid] .== pm_ac)
        @constraint(model, qn_ac[grid] .== qm_ac)
    end
    
    # Voltage coupling constraints:
    for i in 1:nconvs_dc
        grid = Int(conv_dc[i, 3])      
        bus_index = Int(conv_dc[i, 2])  
        @constraint(model, vn2_ac[grid][bus_index] == v2s_dc[i])
    end
